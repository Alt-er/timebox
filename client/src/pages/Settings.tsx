...
export default function Settings() {
  ...
  useEffect(() => {
    window.ipcRenderer.invoke('readConfig').then((config) => {
      setClientConfig({
        ...
        similarityThreshold: config.similarityThreshold || 80,
        maxImagesPerPush: config.maxImagesPerPush || 5,
  ...
  const handleSaveClientConfig = async () => {
    try {
      await window.ipcRenderer.invoke('writeConfig', clientConfig);
      showToast('客户端配置保存成功', 'success');
    } catch (error) {
          ...
          <div className="card-body">
            ...
            <div className="form-control mt-4">
              ...
              <input
                ...
                step="1"
              />
              <div className="text-center mt-2">{clientConfig.similarityThreshold}%</div>
            </div>

            <div className="form-control mt-4">
              <label className="label">
                <span className="label-text">单次最大推送图片数量</span>
                <span className="label-text-alt">每次最多推送多少张图片到服务器</span>
              </label>
              <input
                type="number"
                className="input input-bordered"
